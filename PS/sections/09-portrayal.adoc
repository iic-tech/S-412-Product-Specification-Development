...
The S-412 feature catalogue defines all features requiring portrayal and their geometric primitives. Currently S-412 only defines features which have a Polygon ("Surface" in S-100 terms) geometry. These are encoded using GML Polygons.
...
| Wind Warning | Wind Warning Threshold
...
| Restricted Visibility Warning | No conditional portrayal
|===
...
The portrayal catalogue model is defined in S-100 Part 9. The S-412 Portrayal catalogue is provided in Annex E and will be available in an XML document which conforms to the S-100 XML Portrayal catalogue Schema. It is structured as follows:
