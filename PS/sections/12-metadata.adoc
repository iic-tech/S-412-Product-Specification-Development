...
#_Use of additional language TBC_#
...
image::../images/figure-metadata_overview.png[UML diagram showing an overview of the S-412 Metadata]

[[fig-metadata-details]]
.S-412 Metadata Details
image::../images/figure-metadata_details.png[UML diagram showing the detailled S-412 Metadata]
...
The S-412 metadata description is a subset of the metadata described in S-100 Part 17, which is a profile of the ISO 19115 standard. The S-412 metadata model restricts the S-100 metadata model to its core elements; S100_ExchangeCatalogue, S100_DatasetDiscoveryMetadata, S100_CatalogueDiscoveryMetadata and S100_SupportFileDiscoveryMetadata. Moreover, the S100_DatasetDiscoveryMetadata is further restricted to remove attributes that are not relevant to a Marine Weather Warning service.

==== S100_DatasetDiscoveryMetadata

Dataset discovery metadata for datasets is described in this table.
...
#_References to support file discovery metadata are not permitted because S-412 does not use support files._#
...
|0..1
|CharacterString
|Short description of the area covered by dataset. For example, a harbour or port name between two named locations.
...
|1
|Boolean
|True: indicates a compressed dataset resouce +
False: indicates an uncompressed dataset resource

|Attribute
|dataProtection
|Indicates if the resource is encrypted
|1
|Boolean
|True: indicates an encrypted dataset resource +
...
|0..1
|Integer
|When a dataset is initially created, the edition number 1 is assigned to it. The edition number is increased by 1 at each new Edition +
#Further discussion is needed on whether or not updated datasets should be allowed#
...
#TBC if S-412 uses Data Coverage feature#
...
NOTE:: replacedData and dataReplacement: The intended use of the attributes replacedData and dataReplacement could be, for example, to provide a mechanism for service providers to build automation when providing replacement data sets to customers within existing subscription periods.
...
|0..1
|S100_TemporalExtent
|The remarks for temporalExtent in the dataset discovery block (S100_DatasetDiscoveryMetadata) apply, except that their scope is the individual coverage and not the dataset as a whole
...
#Scale policy on scales to be considered by WMO#
...
#Suggest adding info saying that grid resolution is of no interest (not populated) for S-412#
...
#Notes above come from S-411. WMO to consider what is needed for S-412#
...
#To be reviewed by WMO if Support Files are not used in S-412#
